...
class MujocoMulti(MultiAgentEnv):
    def __init__(self, batch_size=None, **kwargs):
        ...
        # ---- DORAEMON domain-randomization state (disabled unless set) ----
        self._dr = None
        self._dr_enabled = False
        self._dr_version = -1
        self._dr_dynamics = None
        self._dr_return = 0.0

        pass

    # ====================================================================
    # DORAEMON domain randomization (no-op for every other algorithm)
    # ====================================================================
    def set_dr_distribution(self, spec, version=0):
        """Install a per-episode dynamics-randomization distribution.

        Called by the DORAEMON vec-env command. ``spec`` is a list of per-dim
        dicts (param, m, M, a, b, [indices]); pass None/[] to disable DR.
        """
        if spec is None or len(spec) == 0:
            self._dr_enabled = False
            return
        if self._dr is None:
            from harl.envs.mamujoco.dynamics_randomizer import DynamicsRandomizer

            self._dr = DynamicsRandomizer()
        self._dr.set_distribution(spec, version)
        self._dr_version = int(version)
        self._dr_enabled = True

    def _apply_dr(self):
        """Sample dynamics from the current distribution and apply to the model."""
        rng = getattr(self.env, "np_random", None)
        self._dr_dynamics = self._dr.sample(rng)
        self._dr.apply(self.env.model, self._dr_dynamics)
        self._dr_return = 0.0
    ...
    def step(self, actions):
        ...
        # ---- DORAEMON: accumulate return, report dynamics+return on done ----
        if self._dr_enabled:
            self._dr_return += float(np.asarray(reward_n).reshape(-1)[0])
            if done_n:
                info["dr_dynamics"] = (
                    self._dr_dynamics.tolist()
                    if self._dr_dynamics is not None
                    else None
                )
                info["dr_return"] = float(self._dr_return)
                info["dr_version"] = int(self._dr_version)
    ...
    def reset(self, **kwargs):
        ...
        self.steps = 0
        # DORAEMON: sample and apply new episode dynamics before the env forwards
        if self._dr_enabled:
            self._apply_dr()
        self.timelimit_env.reset()
