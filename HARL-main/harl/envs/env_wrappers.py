...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        elif cmd == "set_dr_distribution":
            # DORAEMON: install a per-episode dynamics-randomization distribution
            spec, version = data
            if hasattr(env, "set_dr_distribution"):
                env.set_dr_distribution(spec, version)
            remote.send(True)
        else:
            raise NotImplementedError
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def set_dr_distribution(self, spec, version=0):
        """DORAEMON: broadcast a dynamics-randomization distribution to all envs."""
        for remote in self.remotes:
            remote.send(("set_dr_distribution", (spec, version)))
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def set_dr_distribution(self, spec, version=0):
        """DORAEMON: install a dynamics-randomization distribution on each env."""
