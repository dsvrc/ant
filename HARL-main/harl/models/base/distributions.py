...
class DiagGaussian(nn.Module):
    ...
    def __init__(
        ...
        if args is not None:
            ...
            self.std_x_coef = 1.0
            self.std_y_coef = 0.5
        # O-MAX rung O4: flag-gated Gaussian-std FLOOR (default 0 = off), an
        # exploration/excitation floor. Purely additive — with std_floor <= 0 the
        # distribution is bit-identical to before.
        self.std_floor = float(args.get("std_floor", 0.0)) if args is not None else 0.0
    ...
    def forward(self, x, available_actions=None):
        ...
        if self.std_floor > 0.0:
            action_std = action_std.clamp(min=self.std_floor)
