"""Runner for PACT-1 on GRF-NS.  An arm selector and a logger, nothing else.
...
--------------------------------------------------------------------------------
    blind      `--algo mappo` (or happo) inside the dial.  No estimator.
    pactoff    trust forced to 0.  BIT-IDENTICAL to blind: the correction is
               exactly 0.0 and no RNG is consumed, so the enacted rotations,
               the trajectory and the training data are the blind arm's.  The
               instrumented honest baseline.
    pact       trust fixed at 0.9, gated by prediction confidence, after warmup.
    fixed      an alias of pact on this channel (see below).
    oracle     handed the TRUE swerve -- the ceiling.  On this channel it cancels
               exactly, so its trajectory is the sigma = 0 trajectory.
    intercept  peer channels deleted, everything else identical.  On (C) it can
               only track the fleet mean; on (B) it is the whole answer.

Learned trust (P-6.2) is not implementable on an invertible action-space
channel -- the correction is a deterministic transform of the sampled action,
so ``d log pi / d g = 0`` -- which is why ``fixed`` and ``pact`` coincide here.
``simple_ns`` has the same limitation and states it; so does the README.
...
_COLS = [
    ...
    "pred_err", "corr_abs", "clip_frac", "rows", "diverged", "bounded", "corr_clipped",
    # is it WINNING?
...
_KEY = {
    ...
    "corr_abs": ("ns_c", abs), "clip_frac": ("ns_clip_frac", None),
...
class PactGrfMixin(object):
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if oracle:
            # the ceiling relies FULLY on the true swerve; pact keeps P-5.1's 0.9
        ...
        print("[PACT-GRF] arm=%s  trust=%s (g=%.2f)  oracle=%d  intercept=%d  "
    ...
    def insert(self, data):
        ...
        for col, (key, fn) in _KEY.items():
            ...
            if fn is not None:
                v = [fn(x) for x in v if isinstance(x, (int, float, np.floating, np.integer))
                     and np.isfinite(x)]
            if v:
        ...
        for t, th in enumerate(infos):
            ...
            if np.all(dones[t]):
                ...
                if isinstance(row, dict):
                    self._wins.append(1.0 if float(row.get("score_reward", 0.0)) > 0 else 0.0)
        super().insert(data)
    ...
    def _write_row(self):
        ...
        if self._rollout % li == 0:
            print("[PACT-GRF] step=%d arm=%s | sigma=%.2f A=%.2f placebo=%.2f d=%.4f "
                  "k=%.3f | fit=%.3f bcos=%.3f cond=%.1f trust=%.3f/%.3f pred_err=%.4f "
                  "| dial_live=%.0f harmed=%.0f rot_sent=%.0f | win=%.3f ret=%.3f"
                  % (row["env_step"], self.arm, row["sigma"], row["A"], row["placebo"],
                     row["d"], row["k_abs"], row["fit_gain"], row["beta_cos"],
                     row["cond_psi"], row["trust_pol"], row["trust_app"], row["pred_err"],
                     row["dial_live"], row["harmed_steps"], row["rot_sent"],
                     row["win_rate"], row["ep_return"]), flush=True)
            self._warn(row)
        for c in self._acc:
            self._acc[c] = []
        self._eps, self._lens, self._wins = [], [], []
    ...
    def _warn(self, row):
        if row["sigma"] > 0 and row["dial_live"] == 0.0 and self._rollout > 2:
            print("[PACT-GRF][REFUSE] severity is %.2f but the dial never produced a "
        ...
        if np.isfinite(row["x_std"]) and row["x_std"] < 1e-6 and self._rollout > 2:
            print("[PACT-GRF][INERT] the lane channels are not varying: there is "
                  "nothing to identify (gate 5).")
        if self.arm not in ("pactoff", "oracle") and np.isfinite(row["fit_gain"]) \
                and row["fit_gain"] < 0.05 and self._rollout > 25 and row["sigma"] > 0 \
                and self.arm != "intercept":
            print("[PACT-GRF][GATE 6] rolling fit_gain %.3f is below the floor: the "
                  "reduction is not holding here." % row["fit_gain"])
        if np.isfinite(row["cond_psi"]) and row["cond_psi"] > 1e3 and self._rollout > 25:
            print("[PACT-GRF][GATE 7][WARN] design-matrix condition number %.0f: beta "
                  "can be USED but not DECOMPOSED -- narrow the claim." % row["cond_psi"])
        if np.isfinite(row["diverged"]) and row["diverged"] > 0:
            print("[PACT-GRF][WARN] %.0f non-finite predictions so far -- trust was "
                  "zeroed there (floor property)." % row["diverged"])
        if self.arm != "pactoff" and np.isfinite(row["trust_app"]) \
                and row["trust_app"] <= 1e-6 and self._rollout > 10:
            print("[PACT-GRF][ASLEEP] applied trust is 0 -- the confidence gate has "
...
def _mk(arm, host):
    return type("PactGrf_%s_%s" % (arm, host.__name__), (PactGrfMixin, host), dict(arm=arm))


# MAPPO host (the standard GRF baseline) and HAPPO host.
GRF_ARMS = {
