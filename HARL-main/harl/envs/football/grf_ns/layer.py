...
class SeverityMixin(object):
    ...
    def _squad(self, raw):
        """Positions of the controlled players, who has the ball, and the
        engine's sticky headings -- all public, all from the last observation."""
        n = self.n_agents
        pos = self._last_pos.copy()
        owner = -1
        engine_head = np.full(n, -1, dtype=np.int64)
        if raw is None:
            return pos, owner, engine_head
    ...
    def _ensure_references(self, pos, owner=-1):
        ...
        if self._committed_load_norm is not None:
            ...
            if abs(load_norm - computed) > 0.01 * max(computed, 1e-9):
                print("[GRF-NS][WARN] committed ns_load_norm=%.6f but the spawn geometry "
                      "gives %.6f -- the yaml value belongs to a different scenario; sigma "
                      "does not mean what the paper says it means until this is fixed."
                      % (load_norm, computed))
        else:
            load_norm = computed
        ...
        if not self._banner_done:
            ...
            st = self.coupling.operator_stats(pos, np.full(self.n_agents, 4), recv0)
            print("[GRF-NS] operator at spawn (all heading right, ball owner recv): zero_diag=%s spread=%.3f "
                  "ratio=%.1fx asym=%.3f links=%d"
    ...
    def reset(self):
        ...
        pos, owner, _ = self._squad(self._raw)
